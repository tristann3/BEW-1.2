...
class ColorTests(TestCase):
    ...
    def test_color_results_empty(self):
        # TODO: Fill in this function to test the color route with no color.
         ...
         self.assertEqual(result.status_code, 200)
         expected_page_text = ''
         self.assertEqual(expected_page_text,'')
#######################
# Froyo Tests
#######################
class FroyoTests(TestCase):
    def test_froyo_results_scenario1(self):
        ...
        # specific scenario.
        result = app.test_client().get('/froyo_results?flavor=green&toppings=rocks')
        self.assertEqual(result.status_code, 200)
        result_page_text = result.get_data(as_text=True)
        expected_page_text = f'You ordered green flavored Fro-Yo with toppings rocks!'
        self.assertEqual(expected_page_text,result_page_text)
    def test_froyo_results_scenario2(self):
        ''' This tests the route when no topppings are added '''
        result = app.test_client().get('/froyo_results?flavor=green')
        self.assertEqual(result.status_code, 200)
        result_page_text = result.get_data(as_text=True)
        expected_page_text = f'You ordered green flavored Fro-Yo with toppings !'
        self.assertEqual(expected_page_text,result_page_text)
